...
class SupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str, tokenizer: transformers.PreTrainedTokenizer):
        ...
        rationale_targets = [f"{example['rationale']}" for example in list_data_dict]

        output_input, output_no_input = OUTPUT_DICT["prompt_input"], OUTPUT_DICT["prompt_no_input"]
        holder = f"{tokenizer.unk_token}" * 96
        output_sources = [
            holder + (output_input.format_map(example) if example.get("input", "") != "" else output_no_input.format_map(example))
            for example in list_data_dict
        ]
