...
from model_utils import PipeTrainer
...
RATIONALE_DICT = {
    "prompt_input": (
        ...
        "Please depict this instruction and the following input, and provide relevant factual information in real world as much as possible.\n\n"
        "### Instruction:\n{instruction}\n\n### Input:\n{input}\n\n### Rationale:"
    ),
    "prompt_no_input": (
        "Below is an instruction that describes a task. "
        "Please depict this instruction and provide relevant factual information in real world as much as possible.\n\n"
        "### Instruction:\n{instruction}\n\n### Rationale:"
    ),
}
OUTPUT_DICT = {
    "prompt_input": (
        "Below is an instruction that describes a task, paired with an input and a rationale that provides further context. "
        "Write a response that appropriately completes the request.\n\n"
        "### Instruction:\n{instruction}\n\n### Input:\n{input}\n\n### Rationale:\n{rationale}\n\n### Response:"
    ),
    "prompt_no_input": (
        "Below is an instruction that describes a task, paired with a rationale that provides further context. "
        "Write a response that appropriately completes the request.\n\n"
        "### Instruction:\n{instruction}\n\n### Rationale:\n{rationale}\n\n### Response:"
    ),
}
...
class SupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str, tokenizer: transformers.PreTrainedTokenizer):
        ...
        list_data_dict = list_data_dict[:int(len(list_data_dict) * 0.6)]
        ...
        rationale_targets = [f"{example['rationale']}{tokenizer.eos_token}" for example in list_data_dict]
...
def train():
    ...
    trainer = PipeTrainer(model=model, tokenizer=tokenizer, args=training_args, **data_module)
    trainer.train()
    trainer.save_state()
